class NodeMaster {
  ...
  NodeMaster(int _densityX, int _densityY, int _densityZ, float _x, float _y) {
    ...
    float shapeSizeX = nodeSize*densityX + (densityZ*2);
    float shapeSizeY = nodeSize*densityY + (densityZ*2);
    _x = _x - shapeSizeX/2;
    _y = _y - shapeSizeY/2;
    ...
    for (int z = 0; z < densityZ; z++) {
      for (int x = 0; x < densityX; x++) {
        for (int y = 0; y < densityY; y++) {
          float _nodeX = (nodeSize*x+(z*2)) + _x;
          float _nodeY = (nodeSize*y+(z*2)) + _y;
          PVector pos = new PVector(_nodeX, _nodeY);
          nodes[z][x+y*densityX] = new Node(pos, 80, nodeSize);
        }
      }
    }
  ...
  void update() {
    ...
    for (int z = 0; z < densityZ; z++) {
      ...
      float alpha = map(z, 0, densityZ, 0, 255);
